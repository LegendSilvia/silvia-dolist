...
def build_prompt(todo: Todo) -> str:
    ...
    parts.append("")
    parts.append(
        "Two ways to interact with my list — pick whichever is faster:\n"
        "\n"
        "  PREFERRED: MCP. If you see tools like `list_todos`, `add_todo`, "
        "`edit_todo`, `mark_done`, `delete_todo` in your toolset, use them "
        "directly — atomic, no shell quoting, no copy-paste round-trip with "
        "me. For destructive ops (delete, mark-done-irreversibly) check "
        "with me first; non-destructive ops you can just run.\n"
        "\n"
        "  FALLBACK: CLI. If MCP isn't registered, you're in a terminal "
        "where `todo` is on PATH. Propose commands; I'll run them. Don't "
        "run destructive shell commands yourself."
    )
    parts.append("")
    parts.append(
        "Either way, the surface is:\n"
        "  read   todo /list  [--all|--tag X|--project P|--overdue|--today]\n"
        "         todo /show <id>\n"
        "  write  todo /add <text>           new todo. text is NL-parsed for\n"
        "                                    dates, #tags, @project, p1|p2|p3\n"
        "                                    (e.g. 'buy milk tmr #shop @home p2')\n"
        "         todo /done <id>            mark complete\n"
        "         todo /undo <id>            mark incomplete\n"
        "         todo /del <id>             delete (TUI asks y/n)\n"
        "         todo /edit <id> <field> <value>   replaces the field.\n"
        "                                    fields: text, description, due,\n"
        "                                    due_time, priority, tags, project,\n"
        "                                    done. value 'none' clears.\n"
        "         todo /note <id> <text>     APPENDS a timestamped note to the\n"
        "                                    description without clobbering\n"
        "                                    prior content. Use this when\n"
        "                                    leaving notes on a todo across\n"
        "                                    multiple turns.\n"
        "\n"
        "When updating description, prefer /note (or read existing then\n"
        "edit_todo with the merged value via MCP) — /edit description\n"
        "REPLACES, which loses prior context."
    )
    parts.append("")
    parts.append(
